...
// @exclude      https://api.bitbucket.org/*
// @exclude      https://api.trello.com/*
...
(function () {
  ...
  function isAtlassianApiUrl(url) {
    ...
    try {
      ...
      return pathname.startsWith('/rest/')
        ...
        || pathname.startsWith('/gateway/graphql');
    } catch {
      return false;
    }
  }
  ...
  // Bare "403" and "401" are intentionally excluded: Jira/Confluence issue IDs
  // like "ABC-401" appear in page titles and would cause false-positive redirects
  // because \b matches the word boundary after the "-" separator. HTTP 401/403
  ...
  const BROKEN_TITLE_RE = /\b(forbidden|unauthorized|unauthorised|unauthenticated|not authenticated|not authorized|not authorised|access denied|sign in|log in|not logged in|session expired|authentication required|authentication failed|session timed out|session has timed out|signed out|logged out|not signed in|token expired|invalid session|session invalidated|session ended|session has ended|session no longer|no longer authenticated|credential expired|requires authentication|re-?authenticate|saml authentication failed|sso authentication failed|oidc authentication failed|openid connect authentication failed|single sign-on required|idp redirect required|identity provider error|xsrf check failed|xsrf security token missing or incorrect|csrf check failed|csrf token invalid|you have been inactive|inactive for too long)\b/i;
  ...
  function pageLooksBroken() {
    ...
    for (let i = 0; i < overlayCount; i++) {
      try {
        ...
        // Skip overlays that are hidden from the user — auth errors only matter
        // when they are actually visible. Skipping hidden overlays avoids false-
  ...
  function scheduleRetryAfterApiError() {
    ...
    function next() {
      ...
      apiRetryHandle = setTimeout(() => { try { redirectOnce(); } finally { i++; next(); } }, API_AUTH_RETRY_DELAYS[i]);
    }
    next();
  }
  ...
  function startRetryLoop(resetFailures = false, resetApiAuth = resetFailures, resetPageContext = resetFailures) {
    ...
    try {
      observer.observe(observeTarget, {
        ...
        subtree: true,
        attributes: true,
        // 'inert' is included alongside 'aria-hidden' and 'hidden': Atlassian SPAs
        // may reveal auth-error dialogs by removing the inert attribute rather than
        // toggling aria-hidden/hidden, and the overlay visibility guard in
        // pageLooksBroken() already skips inert elements — so attribute mutations
        // that remove inert must also trigger a redirect check.
  ...
  window.addEventListener('pageshow', (e) => {
    try {
      ...
      console.warn(`${LOG_PREFIX} pageshow error:`, err);
    }
  });
  ...
  if (document.readyState === 'loading') {
    // Pass resetApiAuth=false so any API 401/403 detected at document-start
    // (before DOMContentLoaded) is preserved into the first retry loop — the
    // same rationale as the visibilitychange handler's startRetryLoop(true, false).
    document.addEventListener('DOMContentLoaded', () => startRetryLoop(true, false), { once: true });
  } else {
    startRetryLoop(true);
  }
