...
(function () {
  ...
  const BROKEN_TITLE_RE = /\b(403|401|forbidden|unauthorized|unauthenticated|not authenticated|not authorized|access denied|sign in|log in|session expired|authentication required|authentication failed|session timed out|signed out|logged out|not signed in|token expired|invalid session|credential expired|requires authentication|re-?authenticate|xsrf check failed|xsrf security token missing or incorrect|csrf check failed|csrf token invalid)\b/i;
